...
function CreatorsTab({ currentUserId, onCountChange }: { currentUserId: string; onCountChange: (n: number) => void }) {
  ...
  async function approve(req: CreatorRequest) {
    ...
    // Notify the applicant by email (best-effort)
    try {
      const { data: { session } } = await supabase.auth.getSession();
      await fetch('https://api.xlcoverage.com/api/creator-decision', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${session?.access_token}`,
        },
        body: JSON.stringify({ userId: req.user_id, name: req.name, decision: 'approved' }),
      });
    } catch (e) {
  ...
  async function decline(req: CreatorRequest) {
    ...
    try {
      ...
      await fetch('https://api.xlcoverage.com/api/creator-decision', {
        ...
        body: JSON.stringify({ userId: req.user_id, name: req.name, decision: 'rejected', reason: note }),
      });
    } catch (e) {
